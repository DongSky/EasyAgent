...
- **自主完成任务**：对话办事默认交给一个带完整工具集的自主 Agent：搜索和读文档、在本机终端写脚本、缺少节点时写代码并测试发布、读接口文档后自己定义 HTTP 节点、把独立子任务派给并行子 Agent、把重复任务保存为可复用流程。工具出错会作为观察结果回给它继续修正；任务结束自动写入记忆并沉淀 `learned-*` 技巧，见[自主执行架构](docs/AUTONOMY.md)与[全新任务如何完成](docs/AUTONOMOUS_TASKS.md)。
...
- **默认媒体节点**：内置文字生图、参考图编辑、视频提交、等待结果和参考图上传。选择自己的服务连接即可使用，无需导入演示流程，见[默认媒体节点](docs/DEFAULT_MEDIA_NODES.md)。
...
**写 Python 脚本或从命令行执行**：先运行[无需模型和服务的最小示例](docs/FIRST_STEPS.zh-CN.md)。准备修改框架时，按[源码阅读路线](docs/CODE_MAP.zh-CN.md)追踪一次执行。**使用浏览器画布**：按下面的工作室步骤操作。两者共用后端运行器，App 独立安装。
...
还没配模型也可以先发需求或创建助手：任务、需求和附件会保存，并提示需要接入的模型或服务。已有文字模型但缺生图等能力时，会先保存流程草稿；补齐连接后回到原对话，Agent 会识别新能力、重新验证并继续处理。
...
**本机终端和文件处理已内置，不需要额外 API Key。** 桌面版和本机 `easyagent studio` 首次启动会创建工作目录并启用终端，Agent 可编写 Python 脚本、运行命令、导入导出附件；命令仍经过任务审批。公开图片链接可直接下载为完整原图附件，并实际解码核验宽高。已手动关闭的设置会保留。工作目录位置与使用方式见[本机执行说明](docs/USER_GUIDE.zh-CN.md#内置终端与文件处理)。
