...
1. 进入「设置 → 模型与服务」，填写模型地址、模型名称和自己的 API Key，保存并测试。
...
- **图片生成 / 编辑 API**：用于文字生图、参考图生成和图片修改。纯文字生图需要生成接口；本项目的参考图案例需要支持图片上传的编辑接口。选择服务时确认它支持的模型、尺寸和文件格式；通过「模型与接口目录」添加对应操作为节点，或按[生图教程](docs/GETTING_STARTED.zh-CN.md)配置。
