...
export function runHistory({ watch, stopWatch, escape, statuses, flash }) {
  ...
  return function render(root, rows, emptyHTML) {
    const previous = lists.get(root) || new Map(),
      current = new Map();
    for (const run of rows) {
      let entry = previous.get(run.id);
      if (!entry) {
        const card = document.createElement('article');
        card.className = 'run-entry';
        card.dataset.runEntry = run.id;
        const header = document.createElement('div');
        header.className = 'run-row';
        const panel = document.createElement('div');
        panel.className = 'run-details';
        panel.hidden = true;
        panel.id = root.id + '-result-' + run.id;
        panel.setAttribute('role', 'region');
        card.append(header, panel);
        entry = { card, header, panel };
      }
      ...
      button.onclick = async () => {
        ...
        try {
          await watch(run.id, panel);
        } catch (error) {
          if (panel.isConnected && !panel.hidden) {
            panel.textContent = '加载失败，请收起后重试。';
            flash(error.message);
          }
        }
      };
