...
export function workspaceChat({api,escape,flash,showTab,renderRun,stopWatch,loadWorkflow,token,download,statuses}) {
  ...
  function ensureCard(turn){
    ...
    root.innerHTML='<div class="chat-user-message"><small>你</small><div data-user></div><div class="chat-sent-files" data-sent-files></div></div><div class="chat-task-card"><div data-card-heading></div><div data-graph></div><div data-choices class="chat-choices"></div><div class="chat-result-message" data-reply></div><div class="chat-result-media" data-media></div><div class="chat-task-actions"><button class="text-button" data-details-toggle aria-expanded="false">展开步骤与结果</button><button class="text-button" data-edit hidden>在画布中打开 ↗</button></div><div class="chat-task-details" data-details hidden></div></div>';
  ...
  async function paint(c){
    ...
    for(const turn of c.turns){
      ...
      const labels={queued:'已收到',routing:'正在匹配合适的流程',building:'正在创建新流程',executing:'正在执行',completed:'处理完成',clarification:'需要补充一点信息',answered:'回复',failed:'处理遇到问题',cancelled:'已停止'};
      ...
      const message=c.messages.find(m=>m.turn_id===turn.id&&m.role==='assistant');find('[data-reply]').innerHTML=formatChat(message?.content||(['clarification','failed'].includes(state.phase)?state.message:''),escape);
