export function searchSettings({page,api,flash,load}) {
  const card=document.createElement('section');card.className='panel search-settings';
  ...
  const form=card.querySelector('form'),status=card.querySelector('[data-search-status]'),result=card.querySelector('[data-search-result]');
  ...
  function fields(){
    ...
    form.elements.api_key.placeholder=current?.credential_source==='saved'?'已保存；留空保留，输入新密钥可替换':'输入搜索 API Key';
    card.querySelector('[data-search-env]').hidden=mode()!=='environment';
    form.elements.api_key_env.required=mode()==='environment';
    card.querySelector('[data-test-current]').disabled=busy||!current?.active||!current?.configured;
  }
  form.oninput=()=>{dirty=true;};form.elements.credential_mode.onchange=()=>{dirty=true;fields();};
  async function refresh(){
    const connection=await api('/v1/studio/search/tinyfish');current=connection;
    status.textContent=connection.configured&&connection.active?
      connection.credential_source==='saved'?'已连接 · 已保存密钥':connection.credential_source==='environment'?'已连接 · 使用服务端环境变量':'已连接 · 凭证未持久保存':
      '尚未连接';
    if(!dirty){form.elements.endpoint.value=connection.endpoint;form.elements.api_key_env.value=connection.api_key_env||'TINYFISH_API_KEY';}
    fields();
  }
  async function test(){
    ...
    while(Date.now()<deadline){
      ...
      if(run.status==='succeeded'){
        result.textContent='测试通过 · 返回 '+run.steps[0].output.results.length+' 条结果。';return;
      }
      if(['failed','cancelled','needs_attention'].includes(run.status))throw Error('搜索测试失败：'+(run.steps[0].error||run.status)+'。请检查密钥和服务地址；任务记录中保留了测试详情。');
      await new Promise(resolve=>setTimeout(resolve,500));
    }
    result.textContent='测试仍在执行，可到任务记录查看；不会重复提交。';
  }
  async function action(fn){
    if(busy)return;busy=true;card.querySelectorAll('button').forEach(b=>b.disabled=true);
    try{await fn();}catch(error){result.textContent=error.message;}
    finally{busy=false;card.querySelectorAll('button').forEach(b=>b.disabled=false);fields();}
  }
  form.onsubmit=async event=>{
    event.preventDefault();const shouldTest=event.submitter?.hasAttribute('data-test-search');
    await action(async()=>{
      const key=mode()==='key'?form.elements.api_key.value.trim():'';
      if(mode()==='key'&&!key&&current?.credential_source!=='saved')throw Error('请填写自己的搜索 API Key');
      result.textContent='正在保存搜索连接…';
      await api('/v1/studio/search/tinyfish','POST',{api_key:key,
        ...(mode()==='environment'?{api_key_env:form.elements.api_key_env.value.trim()}:{}),
        endpoint:form.elements.endpoint.value.trim(),timeout_seconds:current?.timeout_seconds||60});
      form.elements.api_key.value='';dirty=false;await refresh();await load();
      window.dispatchEvent(new Event('eah:connections-changed'));
      result.textContent='搜索连接已保存';
      if(shouldTest)await test();else flash('搜索连接已保存');
    });
  };
