const evidenceText=value=>value==='Executable checks and step receipts'?'已核对输出字段和执行回执':value;
export function goalResult({run,root,api,escape,guard,watch,loadWorkflow,showTab}) {
  if(!run.spec?.metadata?.goal)return;
  const state=run.steps[0]?.state||{},result=run.steps[0]?.output,box=document.createElement('section');box.className='panel';
  const label=result?.goal_status==='completed'?'已通过结果检查':result?.goal_status==='incomplete'?'尚未达到目标':state.paused?'已暂停续跑':state.phase==='need_input'?'需要补充信息':'正在执行与检查';
  ...
  root.prepend(box);
  box.querySelector('[data-goal-plan]').onclick=()=>{loadWorkflow(state.workflow);showTab('workflow')};
  box.querySelector('[data-goal-reuse]')?.addEventListener('click',guard(async()=>{const saved=await api(`/v1/studio/workflows/${state.saved_workflow.id}?revision=${state.saved_workflow.revision}`);loadWorkflow(saved.workflow||saved);showTab('workflow')}));
  const control=async(action,feedback='')=>{await api(`/v1/goals/${run.id}/control`,'POST',{action,feedback});await watch(run.id,root)};
  ...
  box.querySelector('[data-goal-send]').onclick=guard(()=>{const f=box.querySelector('textarea').value.trim();if(!f)throw Error('请填写反馈或补充信息');return control('feedback',f)});
}
