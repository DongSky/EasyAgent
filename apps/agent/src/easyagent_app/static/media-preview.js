...
const types = new Map([
  ['image/png','img'],['image/jpeg','img'],['image/webp','img'],['image/gif','img'],
  ['video/mp4','video'],['video/webm','video'],['audio/mpeg','audio'],['audio/wav','audio'],['audio/ogg','audio']
]);
...
export function bindMedia(root, token, guard) {
  const urls=[];active.set(root,urls);
  root.querySelectorAll('[data-media-preview]').forEach(button=>button.onclick=guard(async()=>{
    const kind=types.get(button.dataset.mediaType);if(!kind)return;
    button.disabled=true;
    try {
      const response=await fetch('/v1/artifacts/'+encodeURIComponent(button.dataset.mediaPreview)+'/content',
      ...
      button.insertAdjacentElement('afterend',media);button.hidden=true;
    } finally {button.disabled=false;}
  }));
}

export async function uploadMedia(file, token, limit = 2_000_000) {
  if(!file.size||file.size>limit)throw Error('请选择不超过 '+Math.floor(limit/1_000_000)+' MB 的文件');
