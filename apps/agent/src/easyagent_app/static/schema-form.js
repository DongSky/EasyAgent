...
export function schemaForm(schema, value = {}) {
  ...
  function resolve(raw) {
    ...
    const target = raw.$ref
      .slice(2)
      .split('/')
      .reduce((v, k) => v?.[k.replace(/~1/g, '/').replace(/~0/g, '~')], definitions);
  ...
  function field(raw, initial, required, title, depth = 0) {
    ...
    if (spec.anyOf?.length === 2 && spec.anyOf.some((s) => s.type === 'null'))
      spec = { ...spec, ...spec.anyOf.find((s) => s.type !== 'null') };
    const wrap = document.createElement('fieldset');
    wrap.style.cssText = 'border:0;padding:8px 0;margin:0';
    const legend = document.createElement('legend');
    legend.textContent = spec.title || title;
    wrap.append(legend);
    if (spec.description) {
    ...
    if (!required && ['object', 'array'].includes(spec.type)) {
      ...
      enabled.checked = initial !== undefined || spec.default !== undefined;
      label.append(enabled, document.createTextNode('填写此项'));
      wrap.append(label);
    }
    ...
    if (spec.type === 'object' && spec.properties) {
      const children = Object.entries(spec.properties).map(([key, child]) => {
        ...
        wrap.append(item.element);
        return [key, item];
      });
      read = () =>
        Object.fromEntries(
          children.map(([key, item]) => [key, item.read()]).filter(([, v]) => v !== undefined)
        );
      ...
      function append(v) {
        const row = document.createElement('div'),
          item = field(spec.items, v, true, '项目', depth + 1),
          remove = document.createElement('button');
        remove.type = 'button';
        remove.textContent = '移除此项';
        row.append(item.element, remove);
        rows.append(row);
      ...
      read = () => {
        if (items.length < (spec.minItems || 0) || items.length > (spec.maxItems ?? Infinity))
          throw Error((spec.title || title) + '的项目数量不符合要求');
        return items.map((x) => x.item.read());
      };
    } else {
      ...
      input.setAttribute('aria-label', spec.title || title);
      input.name = title;
      if (spec.enum) {
        if (!required) {
        ...
        if (type === 'boolean') input.checked = initial ?? false;
          ...
          input.value =
            initial === undefined ? '' : complex ? JSON.stringify(initial, null, 2) : initial;
        ...
        if (complex) {
          const hint = document.createElement('small');
          hint.textContent = '此扩展使用自由结构；高级模式填写 JSON。';
          wrap.append(hint);
        }
      }
      ...
      read = () => {
        ...
        if (complex) {
          try {
            ...
            throw Error((spec.title || title) + '需要有效 JSON');
          }
        }
        return input.value;
      };
    ...
    if (enabled) {
      const previous = read;
      read = () => (enabled.checked ? previous() : undefined);
    }
    return { element: wrap, read };
  }
  const fields = Object.entries(schema.properties || {}).map(([key, spec]) => {
    const item = field(spec, value[key], (schema.required || []).includes(key), key);
  ...
  return {
    ...
    values: () =>
      Object.fromEntries(
        fields.map(([key, item]) => [key, item.read()]).filter(([, v]) => v !== undefined)
      ),
  };
}
