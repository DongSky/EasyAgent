...
const permissionText={
  ...
  unsupported:'当前环境不支持系统通知。请用支持通知的浏览器打开，使用 HTTPS 或本机 localhost 地址。手机和应用内浏览器的支持情况可能不同。',
};

export function systemNotifications({form,api,flash,refresh}){
  const area=document.createElement('div');area.dataset.systemNotice='';area.hidden=true;
  ...
  let busy=false,stopped=false,registration=null;
  // A receipt failure must never repeat the OS action, even in this tab.
  let pendingReceipt=null;
  function permission(){return window.isSecureContext&&'Notification'in window&&'serviceWorker'in navigator?Notification.permission:'unsupported'}
  function device(create=false){
    try{
      let id=localStorage.getItem(DEVICE_KEY);
      if(!/^[a-f0-9]{32}$/.test(id||'')){
        if(!create)return null;
        id=crypto.randomUUID().replaceAll('-','');localStorage.setItem(DEVICE_KEY,id);
      }
      return id;
    }catch{if(create)throw Error('浏览器未允许保存设备标识，请开启本站的本地存储');return null}
  }
  function update(){
    const system=form.elements.kind.value==='system',state=permission();
    area.hidden=!system;
    for(const name of ['url','credential','recipient','idempotent']){
      const input=form.elements[name];input.disabled=system;
      input.closest('label').hidden=system||(name==='recipient'&&!['telegram','slack'].includes(form.elements.kind.value));
    }
    ...
    $('[data-enable-notifications]').textContent=state==='granted'?'已允许系统通知':'允许系统通知';
  ...
  async function worker(){
    ...
    if(!registration.active){
      ...
      await new Promise((resolve,reject)=>{
        const done=()=>{
          if(candidate.state==='activated'){cleanup();resolve()}
          else if(candidate.state==='redundant'){cleanup();reject(Error('通知服务启动失败'))}
        };
        const timer=setTimeout(()=>{cleanup();reject(Error('通知服务启动超时'))},10000);
        const cleanup=()=>{clearTimeout(timer);candidate.removeEventListener('statechange',done)};
        candidate.addEventListener('statechange',done);done();
      });
    }
    return registration;
  }
  ...
  async function drain(){
    ...
    try{
      ...
      const service=await worker(),device_id=device();
      for(let count=0;count<5;count++){
        const {notification:n}=await api('/v1/connections/system/claim','POST',{device_id});
        if(!n)return;
        let outcome='submitted',reason;
        try{
          await service.showNotification(n.title,{
            body:n.text,tag:'eah-'+n.id,renotify:false,data:{target:'/#runs'},
          });
        }catch{
          outcome='failed';reason=permission()!=='granted'?'permission_denied':'notification_error';
        }
        pendingReceipt={id:n.id,body:{device_id,claim_token:n.claim_token,outcome,...(reason?{reason}:{})}};
        await receipt();
      }
    }finally{busy=false}
  }
  async function test(id){
    await enable();
    const queued=await api(`/v1/connections/${encodeURIComponent(id)}/test-system`,'POST',{device_id:device(true)});
    $('[data-notification-result]').textContent='测试通知已入队，正在请求系统显示。请同时查看系统通知中心和下方投递记录。';
    await drain();
    await refresh();
    const delivery=(await api('/v1/connections/deliveries')).find(d=>d.id===queued.delivery_id);
    if(delivery?.status==='submitted')$('[data-notification-result]').textContent='浏览器已接受测试通知，请查看系统通知中心。是否显示横幅由系统设置决定。';
    else if(delivery?.status==='failed')$('[data-notification-result]').textContent=delivery.error||'测试通知提交失败，请检查通知权限。';
  }
  async function save(sendTest){
    if(sendTest)await enable();
    await api('/v1/connections','POST',{
      id:form.elements.id.value,title:form.elements.title.value,kind:'system',device_id:device(true),
    });
  ...
  function bindSaved(root,connections){
    for(const button of root.querySelectorAll('[data-test-system]')){
      const c=connections.find(c=>c.id===button.dataset.testSystem),current=c.device_id===device();
      button.disabled=!current||permission()==='unsupported';
      button.textContent=current?'发送测试通知':'绑定在其他浏览器';
      button.onclick=async()=>{button.disabled=true;try{await test(c.id)}catch(e){flash(e.message)}finally{button.disabled=!current||permission()==='unsupported'}};
    }
  }
  $('[data-enable-notifications]').onclick=async()=>{try{await enable();await drain()}catch(e){flash(e.message)}};
  window.addEventListener('focus',()=>{update();drain().catch(()=>{})});
  window.addEventListener('pagehide',()=>{stopped=true});
  window.addEventListener('pageshow',()=>{stopped=false});
  setInterval(()=>{if(!stopped)drain().catch(()=>{})},4000);
  update();
