...
- **Image generation / editing API**: creates images from text, generates from references, or edits existing images. Text-only generation needs a generation endpoint; this project's reference-image example needs an editing endpoint with image upload support. Check supported models, dimensions, and file formats. Add the operation as a node through Models & API Catalog (`模型与接口目录`), or configure it using the [image tutorial](docs/GETTING_STARTED.en.md).
