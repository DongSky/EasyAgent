...
from .execution import agent, context, model_calls
from .execution.context import _pin_request  # noqa: F401 - historical helper import
...
from .retry_policy import attempt_number, error_info, retry_delay
...
def named_outputs(run):
    ...
    try:
        outputs = resolve(
            mapping, {"$input": run["spec"]["inputs"], **{s["id"]: s["output"] for s in run["steps"]}}
        )
...
class Hub:
    ...
    async def run_job(self, job):
        ...
        try:
            ...
            if job["spec"]["kind"] == "tool" and self.tools.regeneratable_media(
                job["spec"]["target"], job["spec"].get("tool_revision")
            ):
            ...
            if uncertain:
                ...
                info = error_info(exc)
                fatal = (
                    fatal or info["category"] == "configuration" or getattr(exc, "retryable", None) is False
                )
                retry = not fatal and attempt < job["spec"]["max_attempts"]
                if (
                    not fatal
                    and not retry
                    and self.tools.regeneratable_media(
                        job["spec"].get("target", ""), job["spec"].get("tool_revision")
                    )
                ):
                    with self.store.connect() as db:
    ...
    async def execute(self, job):
        ...
        if attempt_number(job) > spec.max_attempts and not job["state"].get("compensation"):
            with self.store.connect() as db:
                regenerate = self.store.automatic(db, job["run_id"]) and self.tools.regeneratable_media(
                    spec.target, spec.tool_revision
                )
            if not regenerate:
    ...
    async def agent(self, job, model, config, force_approval=False):
        return await agent.agent(self, job, model, config, force_approval)

    async def execute_pending(self, job, state, config, force_approval):
        return await agent.execute_pending(self, job, state, config, force_approval)

    async def observed_invoke(self, job, config, name, arguments, slot, force_approval):
        return await agent.observed_invoke(self, job, config, name, arguments, slot, force_approval)

    async def compact_for_model(self, job, state, config, model, *, overflow=False):
        return await context.compact_for_model(self, job, state, config, model, overflow=overflow)

    async def correct_schema_violation(self, job, state, config, model, exc):
        return await agent.correct_schema_violation(self, job, state, config, model, exc)

    async def recover_model_turn(self, job, state, config, model, exc):
        return await agent.recover_model_turn(self, job, state, config, model, exc)

    async def compact_context(self, job, state, limit):
        return await context.compact_context(self, job, state, limit)

    async def generate(self, job, request):
        return await model_calls.generate(self, job, request)
    ...
    async def composite(self, job, spec, arguments):
        ...
        if job["state"].get("child_count") is None:
            ...
            for index, item in enumerate(items):
                ...
                child.inputs.update(
                    {**{k: v for k, v in arguments.items() if k != "items"}, "item": item, "index": index}
                    if spec.kind == "foreach"
                    else item
                )
                self.submit(
    ...
    async def plan_execute(self, job, model, config, state, force_approval):
        return await agent.plan_execute(self, job, model, config, state, force_approval)
